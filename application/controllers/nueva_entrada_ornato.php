...
class Nueva_entrada_ornato extends CI_Controller {
    ...
    public function agregar()
    {
        if($this->input->post())
        {
            ...
            $instock = $this->consultas->getStockOrnatoByName($descripcion);
            ...
            if ($instock == "TRUE") 
            {
                ...
                $stock = $this->consultas->getFolioStockOrnato();
                ...
                $validation = $this->agregar->setEntradaOrnato($data_post);
                ...
                $stock = $this->agregar->updateStockOrnatoByCode($id_stock,$codigo_stock,$descripcion_stock,$unidad_stock,$cant_actual,$departamento_stock);

                if ($validation) 
                {   
                   echo '<script language="javascript">alert("Los datos se han agregado correctamente");</script>';
                    redirect(base_url().'stock_ornato','refresh'); 
                }
                else
                {
                    echo '<script language="javascript">alert("No se han podido cargar los datos, intente de nuevo");</script>';
                    redirect(base_url().'Nueva_entrada_ornato','refresh');  
                } 
            ...
            else
            {
                ...
                $stock = $this->agregar->setStockOrnato($data_stock);
    ...
    {
        $data = $this->consultas->getFolioEntradaOrnato();

        $html = '';

        $html .= '<div class="page-header" ><h1>Entrada de artículos de ornato y limpieza</h1></div>'; 
            $html .= '<div class="span12">';
                $html .= '<form class="form-inline" action="'.base_url().'nueva_entrada_ornato/agregar" method="post">';
                    /************************************************************************
                    ...
                    ************************************************************************/
                    ...
                    $html .= '<div class="col-md-12" id="solicitante_orden">';
                        ...
                        $html .= '<div class="col-md-8">';
                            ...
                            $html .= '<label>&nbsp;&nbsp;Código: &nbsp;</label><input type="text" class="form-control" name="codigo" required id="codigo_entrada_ornato"><br>';
                                ...
                                $html .= '<table class="display data_table2" id="table_refacciones">';
                                    ...
                                    $html .= '<tbody>';
                                        $html .= '<tr>';
                                            ...
                                            $html .= '<td><input type="text" class="form-control"  name="descripcion" id="concepto_entrada_ornato"></td>';
