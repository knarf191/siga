...
class Refacciones extends CI_Controller {
	...
	{
		$data = $this->consultas->getDatosRefacciones();
        $user = $this->session->userdata('user');
        $data_vale = $this->consultas->getPermisosById($user, "insumos");

        $html = '';
        $html .= '<div class="page-header" ><h1>Refacciones</h1></div>'; 
            $html .= '<div class="span12">';
            $agregar_orden = $data_vale['agregar'];
            if ($agregar_orden=="true") 
            {
                $html .= '<a href="'.base_url().'solicitud_refacciones" class="btn btn-success" id="newVale"><i class="fa fa-plus-circle"></i>&nbsp;Nuevo</a>';
            }
                ...
                $html .= '</div>';
                $html .= '<table id="tRefacciones" class="display data_table2">';
                        $html .= '<thead>';
                            $html .= '<tr>';
                                $html .= '<th>Folio</th>';
                            ...
                            if (!empty($data)) 
                            {
                                foreach ($data as $clave => $row) 
                                {
                                    ...
                                    $html .= '<td class="placa">$'.$total.'</td>';
                                        ...
                                        {
                                            $html .= '<button class="btn btn-info" title="Imprimir" id="imprimir_solicitud">';
                                        ...
                                        {
                                            $html .= ' <a href="" class="btn btn-warning" title="Editar" id="editar_solicitud">';
