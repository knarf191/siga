...
class Editar_salida_papeleria extends CI_Controller {
    ...
    public function editar()
    {
        if($this->input->post())
        {
            ...
            $validation = $this->agregar->updateSalidaPapeleria($folio,$fecha,$solicita,$departamento);
            ...
            $cantidad    = $this->input->post('cantidad');  
                ...
                { 
                   ...
                   $query2 = $this->db->query("SELECT * FROM material_salida_papeleria WHERE id = '$folio' AND codigo='".$codigo[$i]."'");
            ...
            $delete_tools = $this->eliminar->deleteMaterialPapeleria($folio);
    ...
    {
        $folio        = $_GET['folio'];
        $data         = $this->consultas->getSalidaPapeleriaByFolio($folio);
        $fecha        = $data['fecha'];
        ...
        $html .= '<div class="page-header" ><h1>Editar salida de artículos de oficina y papeleria</h1></div>'; 
            $html .= '<div class="span12">';
                $html .= '<form class="form-inline" action="'.base_url().'editar_salida_papeleria/editar" method="post">';
                    /************************************************************************
                        ...
                        $html .= '<div class="row" id="lista_salida">';
                            $html .= '<div class="col-md-11">';
                                ...
                                $html .= '<table class="display data_table2" id="table_Outs">';
                                    ...
                                    $data_material = $query = $this->db->query("SELECT * FROM material_salida_papeleria WHERE id = '".$folio."' ");
                                    foreach ($data_material->result_array() as $row)
                                    {
                                        $html .= '<tr>';
                                            ...
                                            $search = $this->db->query("SELECT * FROM stock_papeleria WHERE codigo = '".$row['codigo']."'");
