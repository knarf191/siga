...
class Editar_entrada_computo extends CI_Controller {
    ...
    public function editar()
    {
        if($this->input->post())
        {
            ...
            $data_post = array(
                ...
                $data_entrada     = $this->consultas->getEntradaComputoByFolio($folio);
                ...
                $stock = $this->agregar->updateCantComputoStock($codigo,$cant_act);

                if ($stock) 
                {
                    $validation = $this->agregar->updateEntradaComputo($folio, $data_post);
                    if ($validation) 
                    {   
                       echo '<script language="javascript">alert("Los datos se han actualizado correctamente");</script>';
                        redirect(base_url().'entradas_computo','refresh'); 
                    }
                    else
                    {
    ...
    {
        $folio        = $_GET['folio'];
        $data         = $this->consultas->getEntradaComputoByFolio($folio);
        $codigo       = $data['codigo'];
        ...
        $html .= '<div class="page-header" ><h1>Editar entrada de artículos cómputo</h1></div>'; 
            $html .= '<div class="span12">';
                $html .= '<form class="form-inline" action="'.base_url().'editar_entrada_computo/editar" method="post">';
                    /************************************************************************
                    ...
                    ************************************************************************/
                    ...
                    $html .= '<div>';
                        ...
                        $html .= '<button class="btn btn-primary" id="btnCancelINComputo">&nbsp;Cancelar</button>';
                    $html .= '</div>'; 
