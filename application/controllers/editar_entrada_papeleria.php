...
class Editar_entrada_papeleria extends CI_Controller {
    ...
    public function editar()
    {
        if($this->input->post())
        {
            ...
            $data_post = array(
                ...
                $data_entrada     = $this->consultas->getEntradaPapeleriaByFolio($folio);
                ...
                $stock = $this->agregar->updateCantPapeleriaStock($codigo,$cant_act);

                if ($stock) 
                {
                    $validation = $this->agregar->updateEntradaPapeleria($folio, $data_post);
                    if ($validation) 
                    {   
                       echo '<script language="javascript">alert("Los datos se han actualizado correctamente");</script>';
                        redirect(base_url().'entradas_papeleria','refresh'); 
                    }
                    else
                    {
    ...
    {
        $folio        = $_GET['folio'];
        $data         = $this->consultas->getEntradaPapeleriaByFolio($folio);
        $codigo       = $data['codigo'];
        ...
        $html .= '<div class="page-header" ><h1>Editar entrada de artículos de oficina y papeleria</h1></div>'; 
            $html .= '<div class="span12">';
                $html .= '<form class="form-inline" action="'.base_url().'editar_entrada_papeleria/editar" method="post">';
                    /************************************************************************
