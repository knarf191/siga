...
class Salidas_ornato extends CI_Controller {
    ...
    {
        $data = $this->consultas->getSalidasOrnato();
        $user = $this->session->userdata('user');
        $data_vale = $this->consultas->getPermisosById($user, "inventario");
        
        $html = '';
        $html .= '<div class="page-header" ><h1>Control de salidas de artículos de limpieza y ornato</h1></div>'; 
            $html .= '<div class="span12">';
            $agregar_orden = $data_vale['agregar'];
            if ($agregar_orden=="true") 
            {
                $html .= '<a href="'.base_url().'nueva_salida_ornato" class="btn btn-success" id="newVale"><i class="fa fa-plus-circle"></i>&nbsp;Nuevo</a>';
            }                                           
                    ...
                    $html .= '<tbody>';
                        ...
                        if(!empty($data))
                        {
                            foreach ($data as $key => $row) 
                            {
                                ...
                                $folio = $row->folio;
                                $data_tools = $query = $this->db->query("SELECT * FROM material_salida_ornato WHERE id = '$folio' ");
                                    $pedido="";
                                    ...
                                    $html .= '<td>'.$pedido.'</td>';
                                     ...
                                     $html .= '<td>';
                                        ...
                                        {
                                            $html .= '<button class="btn btn-info" title="Imprimir" id="imprimir_salida_ornato">';
                                        ...
                                        if ($editar_orden=="true") 
                                        {
                                            ...
                                            if ($fecha == $date) 
                                            {
                                                $html .= ' <a href="" class="btn btn-warning" title="Editar" id="editar_salida_ornato">';
