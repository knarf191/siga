...
class Nueva_salida_papeleria extends CI_Controller {
    ...
    public function agregar()
    {
        if($this->input->post())
        {
            ...
            $validation = $this->agregar->setSalidaPapeleria($data);
            ...
            $validation_tools = $this->agregar->setSalidaMaterialPapeleria($folio, $codigo, $descripcion, $cantidad);
            ...
            {   
                ...
                redirect(base_url().'salidas_papeleria','refresh');
            }
            else
            {
                for ($i=0; $i < count($codigo); $i++) 
                { 
                   $query = $this->db->query("SELECT *FROM stock_papeleria WHERE codigo = '".$codigo[$i]."'");
                   $data = $query->row_array();
                   $update = $data['cantidad']-$cantidad[$i];

                   $update  = $this->agregar->updateStockPapeleria($codigo[$i],$update);
                }

                $data = $this->consultas->getFolioOUTPapeleria();
                ...
                redirect(base_url().'vista_previa_salida_papeleria?folio='.$folio,'refresh'); 
            }                   
        }
    }
    ...
    {
        $data = $this->consultas->getFolioSalidaPapeleria();

        $html = '';

        $html .= '<div class="page-header" ><h1>Salida de papelería y artículos de oficina</h1></div>'; 
            $html .= '<div class="span12">';
                $html .= '<form class="form-inline" action="'.base_url().'nueva_salida_papeleria/agregar" method="post">';
                    /************************************************************************
                    ...
                    ************************************************************************/
                    ...
                    $html .= '<div class="col-md-12">';
                        $html .= '<div class="row" id="lista_salida">';
                            $html .= '<div class="col-md-11">';
                                ...
                                $html .= '<table class="display data_table2" id="table_OutsPapeleria">';
                                    $html .= '<thead>';
                                        $html .= '<tr>';
                                            $html .= '<th>Codigo</th>';
                                    ...
                                    $html .= '<tbody>';
                                        $html .= '<tr>';
                                            $html .= '<td><input type="text" class="form-control" name="codigo[]" id="codigo_salida_papeleria"></td>';

                                            $html .= '<td><input type="text" class="form-control" name="descripcion[]" id="descripcion_salida"></td>';
                                       
                                            $html .= '<td><input type="text" class="form-control" id="cantidad_salida_papeleria" disabled></td>';
                                            $html .= '<td><input type="text" class="form-control" name="cantidad[]" id="cantidad_solicitada_papeleria"></td>';    
                                            $html .= '<td class="btnRefacciones"><i class="fa fa-plus-circle fa-lg" id="plusOutPapeleria"></i></td>';                                        
                                        $html .= '</tr>';
                                    $html .= '</tbody>';
                                $html .= '</table>'; 
                                $html .= '<input type="hidden" id="producto">'; 
                                $html .= '<input type="hidden" id="cantidad_sol_papeleria">'; 
                            $html .= '</div>'; 
                        $html .= '</div>'; 
                    $html .= '</div>';     
                    ...
                    $html .= '<div>';
                        ...
                        $html .= '<button class="btn btn-primary" id="btnCancelOUTPapeleria">&nbsp;Cancelar</button>';
                    $html .= '</div>'; 
