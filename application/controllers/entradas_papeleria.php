...
class Entradas_papeleria extends CI_Controller {
    ...
    {
        $data = $this->consultas->getEntradasPapeleria();
        $user = $this->session->userdata('user');
        $data_vale = $this->consultas->getPermisosById($user, "inventario");
        
        $html = '';
        $html .= '<div class="page-header" ><h1>Control de entrada de papeleria y artículos de oficina</h1></div>'; 
            $html .= '<div class="span12">';
            $agregar_orden = $data_vale['agregar'];
            if ($agregar_orden=="true") 
            {
                $html .= '<a href="'.base_url().'nueva_entrada_papeleria" class="btn btn-success" id="newVale"><i class="fa fa-plus-circle"></i>&nbsp;Nuevo</a>';                                            
            }
                                ...
                                foreach ($data as $clave => $row) 
                                {
                                    $html .= '<tr>';
                                        ...
                                        if ($editar_orden=="true") 
                                        {
                                            ...
                                            if ($row->fecha==$date) 
                                            {
                                                $html .= ' <a href="" class="btn btn-warning" title="Editar" id="editar_entrada_papeleria">';
