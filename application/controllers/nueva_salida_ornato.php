...
class Nueva_salida_ornato extends CI_Controller {
    ...
    public function agregar()
    {
        if($this->input->post())
        {
            ...
            $validation = $this->agregar->setSalidaOrnato($data);
            ...
            $validation_tools = $this->agregar->setSalidaMaterialOrnato($folio, $codigo, $descripcion, $cantidad);
            ...
            {   
                ...
                redirect(base_url().'salidas_ornato','refresh');
            }
            else
            {
                for ($i=0; $i < count($codigo); $i++) 
                { 
                   $query = $this->db->query("SELECT *FROM stock_ornato WHERE codigo = '".$codigo[$i]."'");
                   $data = $query->row_array();
                   $update = $data['cantidad']-$cantidad[$i];

                   $update  = $this->agregar->updateStockOrnato($codigo[$i],$update);
                }
                ...
                redirect(base_url().'vista_previa_salida_ornato?folio='.$folio,'refresh'); 
            }                   
        }
    }
    ...
    {
        $data = $this->consultas->getFolioSalidaOrnato();

        $html = '';

        $html .= '<div class="page-header" ><h1>Salida de artículos de limpieza y ornato</h1></div>'; 
            $html .= '<div class="span12">';
                $html .= '<form class="form-inline" action="'.base_url().'nueva_salida_ornato/agregar" method="post">';
                    /************************************************************************
                    ...
                    $html .= '<div class="col-md-12">';
                        $html .= '<div class="row" id="lista_salida">';
                            $html .= '<div class="col-md-11">';
                                ...
                                $html .= '<table class="display data_table2" id="table_OutsOrnato">';
                                    $html .= '<thead>';
                                        $html .= '<tr>';
                                            $html .= '<th>Codigo</th>';
                                    ...
                                    $html .= '<tbody>';
                                        $html .= '<tr>';
                                            $html .= '<td><input type="text" class="form-control" name="codigo[]" id="codigo_salida_ornato"></td>';

                                            $html .= '<td><input type="text" class="form-control" name="descripcion[]" id="descripcion_salida"></td>';
                                       
                                            $html .= '<td><input type="text" class="form-control" id="cantidad_salida_ornato" disabled></td>';
                                            $html .= '<td><input type="text" class="form-control" name="cantidad[]" id="cantidad_solicitada_ornato"></td>';    
                                            $html .= '<td class="btnRefacciones"><i class="fa fa-plus-circle fa-lg" id="plusOutOrnato"></i></td>';                                        
                                        $html .= '</tr>';
                                    $html .= '</tbody>';
                                $html .= '</table>'; 
                                $html .= '<input type="hidden" id="producto">'; 
                                $html .= '<input type="hidden" id="cantidad_sol_ornato">'; 
                            $html .= '</div>'; 
                        $html .= '</div>'; 
                    $html .= '</div>';     
